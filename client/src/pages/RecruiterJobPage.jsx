...
// NEW: Utility function to generate recruiter ID
...
const RecruiterJobPage = () => {
  ...
  const [recruiterId, setRecruiterId] = useState(null); // NEW: Added recruiterId state
  const [copied, setCopied] = useState(false);
  const [error, setError] = useState(null);

  // NEW: useEffect to load/generate recruiter ID on mount
  useEffect(() => {
    let recId = localStorage.getItem("recruiterId");
    if (!recId) {
      recId = generateRecruiterId();
      localStorage.setItem("recruiterId", recId);
    }
  ...
  // UPDATED: handleSubmit with recruiter ID logic
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post("/api/jobs/create", {
        ...formData,
        recruiterId: recId, // Include recruiter ID in request
      });

      if (response.data.success) {
        // Ensure recruiter ID is saved
        localStorage.setItem("recruiterId", response.data.data.recruiterId);
        setRecruiterId(response.data.data.recruiterId);
  ...
  // UPDATED: magicLink generation to include recruiter ID
  const magicLink =
    jobId && recruiterId
      ? `${window.location.origin}/interview/${jobId}?recruiter=${recruiterId}`
      : "";
          ...
          <>
            <div className="text-center mb-12">
              <h1 className="text-5xl font-medium tracking-tighter text-white mb-4">
                Create{" "}
                <span className="text-[#ccff00] font-serif italic">Job</span>{" "}
                Position
              </h1>
          ...
          <>
            ...
            <GlassPanel className="rounded-[2.5rem] p-10 space-y-8 animate-in zoom-in-95 duration-500 delay-150">
              ...
              <div className="flex gap-4 pt-4">
                ...
                >
                  Test Link <ArrowLeft className="w-4 h-4 rotate-180" />
                </button>
              </div>
            </GlassPanel>
          </>
